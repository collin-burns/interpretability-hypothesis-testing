...
class BERTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        tokens_b = []
        while len(tokens_b) == 0:  # to avoid assertion failure later, resample if tokens_b has len 0
...
def main():
    ...
    dataset = BERTDataset(args.train_file, tokenizer, seq_len=args.max_seq_length,
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            output_eval_file = os.path.join(args.output_dir, "eval_results_before_%d.txt" % epoch)
            ...
            logger.info("** ** * Saving fine-tuned model at epoch %d ** ** *" % epoch)
            model_to_save = model.module if hasattr(model, 'module') else model  # Only save the model it-self
            output_model_file = os.path.join(args.output_dir, "tuned_epoch_%d.bin" % epoch)
            if args.do_train:
