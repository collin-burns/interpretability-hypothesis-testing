...
def get_top_k_features(feature_values, k):
    ...
    if selected_features_mask.sum() != k: # Can happen if a feature value is repeated between two features
        print("Warning: Selected {} features when k={}".format(selected_features_mask.sum(), k))
...
def main(args):
    ...
    with sns.axes_style('white'):
        ...
        plt.plot(xs, ys, label="IRT", lw=lws[0], color=colors[0], linestyle=linestyles[0])
        xs, ys = process(mean_one_osft[FDR_IDX], mean_one_osft[TPR_IDX])
        plt.plot(xs, ys, label="OSFT", lw=lws[1], color=colors[1], linestyle=linestyles[1])
        if not args.skip_lime:
            xs, ys = process(mean_one_lime[FDR_IDX], mean_one_lime[TPR_IDX])
            plt.plot(xs, ys, label="LIME", lw=lws[2], color=colors[2], linestyle=linestyles[2])
        xs, ys = process(mean_one_shap[FDR_IDX], mean_one_shap[TPR_IDX])
        plt.plot(xs, ys, label="SHAP", lw=lws[3], color=colors[3], linestyle=linestyles[3])
        ...
        plt.plot(xs, ys, label="OSFT", lw=lws[1], color=colors[1])
        if not args.skip_lime:
        ...
        if not args.skip_l2x:
            ...
            plt.plot(xs, ys, label="L2X", lw=lws[7], color=colors[7], linestyle=linestyles[7])
    ...
    alphas = [0.05, 0.1, 0.2]
    two_sided = [mean_two_irt, mean_two_osft, mean_two_shap]
    one_sided = [mean_one_irt, mean_one_osft, mean_one_shap]
    names = ["IRT", "OSFT", "SHAP"]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--experiment_dir", "-d", type=str, default="{}_disc_experiment{}")
